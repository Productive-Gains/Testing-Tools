    ...
    var keepImagesForReview = function (screenShotFileName, referenceFileName, compareFileName, webDriver, folderName, fileNameSuffix) {
        ...
        getBrowserName(webDriver).then(function (browserName) {
            var failedFolder = path.join(tmpFolder, 'screenshots', 'failed', folderName),
                ...
                screenShotFileName = path.join(failedFolder, process.platform + "-" + browserName + '-screenshot' + now + '-' + fileNameSuffix + '.png'),
                referenceFileName = path.join(failedFolder, process.platform + "-" + browserName + '-reference' + now + '-' + fileNameSuffix + '.png'),
                compareFileName = path.join(failedFolder, process.platform + "-" + browserName + '-compare' + now + '-' + fileNameSuffix + '.png');
            fs.createReadStream(screenShotFileName).pipe(fs.createWriteStream(screenShotFileName));
            fs.createReadStream(referenceFileName).pipe(fs.createWriteStream(referenceFileName));
            fs.createReadStream(compareFileName).pipe(fs.createWriteStream(compareFileName));
            defer.fulfill();
        });
        return defer.promise;
    };
            ...
            webDriver.manage().window().getSize()
                ...
                easyimg.info(screenShotFileName.value).then(function (info) {
                    ...
                    easyimg.crop({
                        ...
                        console.trace('Element picture taken: ' + JSON.stringify(image));
                        defer.fulfill(image);
                    }, function (err) {
                        console.error(err);
                        defer.reject(new Error(err));
