...
module.exports = function (testFolder, tmpFolder) {
     ...
     * @returns {string}
     */
    var yearToSecondFormat = function (date) {
    ...
    var keepImagesForReview = function (screenShotFileName, referenceFileName, compareFileName, webDriver, folderName, fileNameSuffix) {
        ...
        getBrowserName(webDriver).then(function (browserName) {
            var failedFolder = path.join(tmpFolder, 'screenshots', 'failed', folderName),
                now = yearToSecondFormat(new Date()),
                pre = process.platform + "-" + browserName,
                keepScreenShotFileName = path.join(failedFolder, pre + '-screenshot-' + now + '-' + fileNameSuffix + '.png'),
                keepReferenceFileName = path.join(failedFolder, pre  + '-reference-' + now + '-' + fileNameSuffix + '.png'),
                keepCompareFileName = path.join(failedFolder, pre + '-compare-' + now + '-' + fileNameSuffix + '.png');
        ...
        Q.allSettled([
            ...
            compareImages(elementScreenShot.value.path, referenceImageFileName.value).then(function (comparison) {
                if (comparison.comparisonNumber > comparisonThreshold) {
                    keepImagesForReview(
                        elementScreenShot.value.path,
                        referenceImageFileName.value,
                        comparison.compareFileName, webDriver, featureName, subFeatureName);
                }
    ...
    var compareImages = function (screenShotFileName, referenceImageFileName) {
        ...
        exec(cmd, {
            ...
            if (err) {
                console.log('Image comparison aws less than perfect with ' + err + ' points off');
            }
