...
module.exports = function (testFolder, tmpFolder) {
    ...
    var findImageMagickBinDir = function (startDir) {
        var dir = startDir || __dirname + '../bin/imagemagick';
    ...
    var findImageMagickHomeDir = function () {
        var dir = __dirname + '../bin/imagemagick';

        if (!fs.existsSync(dir)){
            console.error('Imagemagick dir does not exist: ' + dir);
        }
    ...
    var setImageMagickEnvSettings = function () {
        ...
        process.env.MAGICK_HOME = findImageMagickHomeDir();
    ...
    var compareImages = function (screenShotFileName, referenceImageFileName) {
        var cmd = 'compare -metric AE -fuzz 0% ' + screenShotFileName + ' ' + referenceImageFileName + ' ' + getTempFileName('unit_test_', '_compare.png'),
